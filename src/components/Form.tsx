...
export const Form = () => {
  ...
  const onSubmit: SubmitHandler<EmailProps> = async data => {
    ...
    const clientEmailHtml = await render(
      <ClientEmail email={data.email} title={data.title} message={data.message} name={data.name} />
    );
    const ownerEmailHtml = await render(
      <OwnerEmail email={data.email} title={data.title} message={data.message} name={data.name} />
    );
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <VStack padding={{ base: 30, sm: 4 }}>
          ...
          <FormControl label="E-メールアドレス" pt={4}>
            <Input
              type="email"
              placeholder="sample-user@exsample.com"
              {...register('email', {
